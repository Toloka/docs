...
Decide whether an annotator can select only one or multiple answer options:
...
The [helper.if](../reference/helper.if.md) component displays an interface element after a specific response is selected. For example, it lets an annotator select correct statements about an image only if the image loaded.
...
Add a button that opens the search results and generate a search query link using the [helper.search-query](../reference/helper.search-query.md) component. To make sure that an annotator clicked on the link and checked its contents, configure validation, as in the example.
