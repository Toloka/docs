...
- To set the desired text length, use the [condition.schema](../reference/condition.schema.md) component instead of [condition.required](../reference/condition.required.md). It won't let the annotator submit a response if the text they entered is longer or shorter than specified.
...
- If the annotator needs to search for information online to complete the task, add the [helper.search-query](../reference/helper.search-query.md) component. The search query can be put together from words taken from the input data.
...
This example is suitable for text search, such as for finding errors in a text. If the annotator finds mistakes, selecting the relevant option opens the text input field.
...
## The annotator adds input fields {#dynamic-field-add}
...
What's unique about this example is that the annotator can add and remove text input fields.
