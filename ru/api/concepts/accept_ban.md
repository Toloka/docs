...
Вы можете блокировать доступ к заданиям для исполнителей, чьи ответы были отклонены заказчиком в ходе отложенной приемки. Для этого задайте значения ключей в массиве `quality_control.configs` в настройках пула.

## Тело запроса {#body}

Доступ к заданиям прекращается, если 40% выполненных заданий были отклонены заказчиком. Условие проверяется впервые после выполнения 10 заданий.
