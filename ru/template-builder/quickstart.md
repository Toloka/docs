...
- В песочнице конструктора
  ...
  [Ссылка]({{ template-builder }}) на песочницу конструктора шаблонов.
...
Если в процессе создания у вас что-то пойдет не так, можете [посмотреть правильный код в песочнице](https://clck.ru/TfQEp).
...
Чтобы пользователи быстрее выполняли ваши задания, добавьте в интерфейс горячие клавиши.
...
Валидация позволяет вам проверять значения, которые вводят пользователи. Следующая часть кода из блока с переключателями проверяет, чтобы исполнители обязательно выбрали один из вариантов.
...
Вы можете дополнить этот код подсказкой пользователю, что нужно исправить для успешной проверки. Добавьте свойство `hint` с текстом подсказки для пользователя так, чтобы этот блок выглядел следующим образом:
```json
{
  "validation": {
    "type": "condition.required",
