...
1. В настройках профиля заказчика перейдите в {% if locale == "ru-ru" %}**Интеграция с внешними сервисами → Настройки прокси**{% endif %}.
...
Для тестирования используйте файл `example.jpg`. Его URL вы можете найти в {% if locale == "ru-ru" %}**Профиль → Интеграция с внешними сервисами**{% endif %}.
...
Да. Настройте оповещения в аккаунте через {% if locale == "ru-ru" %}**Профиль → Уведомления → Выполнение пула или агрегации**{% endif %}{% if locale == "en-com" %}**Profile → Notifications → Pool or aggregation completed**{% endif %}. [Подробнее о настройке оповещений.](../concepts/result-aggregation.md#notification)
...
Установить цену за **страницу** заданий можно на странице редактирования пула. Минимальная цена для обычных пулов составляет 0,001 $.
