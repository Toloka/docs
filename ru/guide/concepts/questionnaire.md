...
Предположим, вы хотите узнать, где завтракают и что предпочитают русскоязычные респонденты. Для этого создадим единственное задание на странице и в [пуле](../../glossary.md#pool-ru): исполнитель видит несколько вопросов с вариантами ответов.
...
1. {% include [toloka-requester-source-edit-interface](../_includes/toloka-requester-source/id-toloka-requester-source/edit-interface.md) %}
    ...
    - обязательная строка `place`, в которую будет записан ответ исполнителя на вопрос <q>Где вы обычно завтракаете?</q>;
    - обязательная строка `food`, в которую будет записан ответ исполнителя на вопрос <q>Что вы чаще всего едите на завтрак?</q>;
    - обязательная строка `drink`, в которую будет записан ответ исполнителя на вопрос <q>Что вы пьете на завтрак (помимо воды)?</q>.
...
1. Для опроса [инструкция](../../glossary.md#task-instruction-ru) не требуется. Пожелания и рекомендации по заполнению опроса поместите в [интерфейс задания](../../glossary.md#task-interface-ru).
    {% note info %}
...
1. В блоке **Цена** установите цену в долларах США за [страницу заданий](../../glossary.md#task-page-ru), например, `0.01`. В опросе страница заданий должна содержать одно задание. Если вам нужна помощь в оценке задач, почитайте про [настройку ценообразования](dynamic-pricing.md#section_wb1_lhl_vlb).
