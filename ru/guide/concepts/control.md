...
Правила контроля качества позволяют получать более точные ответы и ограничивать доступ к заданиям недобросовестным исполнителям. Все правила работают независимо друг от друга.
...
## Готовые наборы правил {#rule-set}

В Толоке существуют готовые наборы правил контроля качества. Сейчас их три:
- **Начальный** — подходит для тех заказчиков, которым необходимо, чтобы задания были выполнены большим числом исполнителей и в короткий срок.
- **Базовый** — подойдет тем, кому необходим баланс между качеством и количеством.
- **Продвинутый** — набор с большим количеством правил, необходим для сложных заданий.
 В таблице представлены правила, которые входят в каждый из наборов.
 |
Заработок
 |
Пропуск заданий
...
## Как настроить контроль качества {#control_how}

Контроль качества можно настроить в [пуле](qa-pool-settings.md) и в [проекте](project-qa.md).
...
При [клонировании проекта](project.md) настройки контроля качества не переносятся.
