...
1. В блоке с надписью **Мы можем сделать контрольные задания для вас** нажмите кнопку **Попробовать бесплатно**.

1. Нажмите кнопку **Заказать**. Данные вашего пула будут отправлены на разметку экспертным исполнителям. Для улучшения качества мы применяем алгоритмы выбора заданий, подходящих в качестве контрольных.
...
1. После завершения разметки результаты будут загружены обратно в пул. Вы увидите сообщение «Контрольные задания готовы» в разделе **Разметить задания** на вкладке **Контрольные**. Вы можете [редактировать](task-markup-by-yourself.md#task-edit) и [удалять](task-markup-by-yourself.md#delete-task) полученные контрольные задания.

{% note info %}

Если по каким-то причинам процесс разметки завершился неудачно, свяжитесь со службой поддержки. Для этого на вкладке контрольных заданий или на странице пула нажмите кнопку **Написать в поддержку**. После решения проблемы вы сможете заказать разметку контрольных заданий снова.
