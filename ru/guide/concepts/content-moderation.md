...
Этот [шаблон проекта](../../glossary.md#project-template-ru) подходит для тех случаев, когда нужно проверить текстовый контент на соответствие правилам (англ. content moderation, CM).
...
1. [Перенести](sandbox.md#export) их в {% if locale == "ru-ru" %}**основную версию Толоки**{% endif %}{% if locale == "en-com" %}**production version**{% endif %}.
...
1. Нажмите кнопку **+ Создать проект** и выберите шаблон **Анализ тональности и модерация контента**.
...
1. Отредактируйте интерфейс задания:
    ...
    **Выходные данные** — типы объектов, которые будут на выходе выполненного задания. Для этого шаблона — один из двух вариантов ответа. При выборе второго ответа откроется список флажков — из них исполнитель должен выбрать подходящие варианты. В других задачах на выходе получают введенный текст или загруженный файл, например.
    ...
    - Выходные данные — строка `quality`, в которую будет записан выбранный вариант из поля <q>Есть ли в тексте нарушения?</q>. Остальные поля — типы нарушений. Используйте этот список полей или настройте его под свои задачи.
...
1. В блоке **Аудитория** добавьте **Фильтры** для отбора исполнителей. Если инструкция, интерфейс задания и сами комментарии на русском языке, воспользуйтесь набором <q>Русскоязычные исполнители</q>. Если планируете анализировать комментарии еще на английском или другом языке, добавьте к ним фильтр: `Языки = Английский`.
...
1. Настройте **Контроль качества**. [Правила контроля качества](control.md) позволяют отсеивать невнимательных исполнителей. Контроль также можно [настраивать и в проекте](project-qa.md). **Отложенная приемка** тут не понадобится.
    ...
    **Как определить время для быстрых ответов?**
    ...
    #### Капчу
    Пример настройки правила. ![](../_images/control-rules/captcha/qcr-captcha_example1.png)
...
1. Настройте обычное или динамическое перекрытие:
    ...
    - [Динамическое перекрытие](../../glossary.md#dynamic-overlap-ru) (incremental relabeling, IRL). Оно поможет вам оптимизировать бюджет относительно получения максимально достоверных ответов. [Пример настроек](dynamic-overlap.md#example).
...
1. Откройте страницу [проекта](../../glossary.md#project-ru), перейдите на вкладку {% if locale == "ru-ru" %}**Обучения**{% endif %}{% if locale == "en-com" %}**Training**{% endif %} и нажмите кнопку {% if locale == "ru-ru" %}**Добавить обучение**{% endif %}{% if locale == "en-com" %}**Add training**{% endif %}.
...
1. Нажмите **Добавить**.
1. Нажмите . Далее добавьте правильный ответ и подсказку для всех загруженных заданий. [Зачем нужна разметка](task-markup-by-yourself.md).
...
1. Сначала запустите обучающий пул, а потом — обычный. Со страницы пула это можно сделать, нажав кнопку ![](../_images/other/b-start-pool.png). Со страницы проекта — ![](../_images/tutorials/content-moderation/pool-action-play.png) напротив названия пула.
