...
Обучающий пул содержит только [обучающие задания](../../glossary.md#training-task-ru). Задания в обучающем пуле не оплачиваются.
...
1. Вы создаете обучающий пул. Автоматически создается [тренировочный навык](../../glossary.md#train-skill-ru). Следующие обучающие пулы используют уже созданный навык.
1. Вы загружаете в пул задания и [размечаете](../../glossary.md#task-markup-ru) их. Задания можно загрузить уже размеченными.
...
1. Откройте страницу [проекта](../../glossary.md#project-ru).
...
1. [Загрузите задания в пул и разметьте их](pool.md) (добавьте правильные ответы и подсказки). Все задания должны быть [обучающими](../../glossary.md#training-task-ru). Можно загрузить уже размеченные задания.
...
{% if locale == "ru-ru" %}**Время на выполнение**{% endif %}{% if locale == "en-com" %}**Time on task**{% endif %} | Время на выполнение [страницы заданий](../../glossary.md#task-page-ru) в секундах.
{% if locale == "ru-ru" %}**Повторное прохождение**{% endif %}{% if locale == "en-com" %}**Retry after**{% endif %} | Количество дней, через которое исполнитель сможет пройти обучение повторно. Если неуказано, [тренировочный навык](../../glossary.md#train-skill-ru) выдается бессрочно, а значение зафиксировано.<br/>[Подробнее о том, как это работает](train.md).
{% if locale == "ru-ru" %}**Выдавать в порядке загрузки**{% endif %}{% if locale == "en-com" %}** Issue in task uploading order**{% endif %} | Если опция включена, задания будут выдаваться исполнителю в том порядке, в котором они указаны в загрузочном [файле](../../glossary.md#tsv-file-definition-ru).
...
После прохождения обучения (успешного или нет) исполнителю будет назначен [тренировочный навык](../../glossary.md#train-skill-ru), а значение зафиксировано.
...
Обучающий пул в статусе {% if locale == "ru-ru" %}<q>Архивный</q>{% endif %}{% if locale == "en-com" %}<q>Archived</q>{% endif %} недоступен для запуска и редактирования.
