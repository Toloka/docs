...
||{% if locale == "ru-ru" %}**Время ожидания закрытия пула, с**{% endif %}{% if locale == "en-com" %}**Keep pool open after completion, sec**{% endif %}| Интервал, в течении которого пул будет оставаться открытым с момента выполнения всех заданий. Минимум — 0 (1 в случае использования динамического перекрытия), максимум — 259 200 секунд (трое суток).
