...
Капча, вместе с правилом [быстрые ответы](quick-answers.md), обеспечивают высокий уровень защиты от роботов.

Чтобы показывать капчу исполнителям, установите {% if locale == "ru-ru" %}[частоту показа капчи](pool_poolparams.md#captcha){% endif %}{% if locale == "en-com" %}[captcha frequency](pool_poolparams.md#captcha){% endif %} в настройках [пула](../../glossary.md#pool-ru).

В Толоке капчу обычно показывают раз на 10 страниц с заданиями, как правило, этого достаточно. Если задания выполняются быстро и капча мешает исполнителям, её можно показывать через каждые 20 страниц.
