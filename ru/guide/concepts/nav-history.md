...
1. Откройте загруженный файл и выберите в качестве разделителя вертикальную черту `|`. Файл может содержать поля:
    ...
    - `pool_id` — идентификатор пула, в котором навык используется как [фильтр](../../glossary.md#filtering) или [правило контроля качества](../../glossary.md#quality-control). Если значение не заполнено, навык был назначен исполнителю вручную.
